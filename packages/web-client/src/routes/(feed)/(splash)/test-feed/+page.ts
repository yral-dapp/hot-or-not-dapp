...
export const load: PageLoad = async ({ url }) => {
	...
	if (cachedVideos.length) {
		throw redirect(
			307,
			`/test-feed/${cachedVideos[0].publisher_canister_id}@${cachedVideos[0].post_id}`
		);
	}
	...
	if ('Ok' in res && res.Ok[0]) {
		throw redirect(
			307,
			`/test-feed/${res.Ok[0].publisher_canister_id.toText()}@${res.Ok[0].post_id}${searchParams}`
		);
	} else {
		throw redirect(307, `/test-feed/no-videos${searchParams}`);
	}
};
