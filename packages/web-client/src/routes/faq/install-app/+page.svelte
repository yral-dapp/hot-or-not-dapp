...
<HomeLayout>
	...
	<svelte:fragment slot="content">
		<div
			...
			<Accordion on:click="{() => toggleAccordion(0)}" expanded="{expandedIndex == 0}">
				<div class="w-full truncate" slot="title">Android</div>
				<div transition:slide|local slot="body" class="py-3 flex flex-col gap-4">
					<span class="text-sm opacity-70">Click on the button below to prompt installation</span>
					<div class="mx-auto">
						<Button on:click="{promptInstall}">Install</Button>
					</div>
					<div class="text-sm opacity-70">Then accept the installation in the popup</div>
					<div class="mx-auto">
						<img src="https://i.imgur.com/nynbYEh.png" alt="" class="h-72" />
					</div>
					<span class="text-xs opacity-40">
						Note: Installation is only supported on Chrome, Firefox and other Chromium based
						browsers
					</span>
				</div>
			</Accordion>
			<Accordion on:click="{() => toggleAccordion(1)}" expanded="{expandedIndex == 1}">
				...
				<div transition:slide|local slot="body" class="py-3 flex flex-col gap-4">
					...
					<div class="text-sm opacity-70">Here is how it'll look in Safari:</div>
					<div class="mx-auto">
